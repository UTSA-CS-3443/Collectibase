...
public class CassetteController {
	...
	/****
	 * The methods below allow you to edit an item by double
	 * clicking the cell. 
	 */
	...
	private void getCassetteFromFile() {
		try {
			...
			while((line = br.readLine()) != null) {
				array = line.split(",");
				cassTable.getItems().add(new CassetteCollection(array[0], array[1], array[2], array[3], 
	...
	public void writeCassetteFile() throws Exception {
		Writer writer = null;
		try {
			File file = new File("cassette.txt");
			writer = new BufferedWriter(new FileWriter(file));
			for (CassetteCollection cass : Storage.allCassettes) {
				String text = cass.getArtist() + "," + cass.getYear() + "," + cass.getAlbum() + "," + cass.getRunningTime() + "," +
							cass.getType() + "," + cass.getWriteProtection() + "," + cass.getAudioOutput() + "," + cass.getEndlessLoop() + "\n";
				writer.write(text);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
